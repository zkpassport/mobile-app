...
public class NFCPassportModel {
    ...
    // Just added:
    public private(set) lazy var fullName : String = { return passportDataElements?["5B"] ?? "?" }()
    ...
    public func getDateOfBirth() -> Date? {
        return convertYYMMDDToDate(from: dateOfBirth)
    }
    public func getExpiryDate() -> Date? {
        return convertYYMMDDToDate(from: documentExpiryDate)
    }
    public func getFirstAndLastName() -> String {
        let firstName = firstName.components(separatedBy: " ")[0]
        return firstName.localizedCapitalized + " " + titleCaseName(lastName)
    }
    public func getSuitableFullName(_ maxSize: Int = 25) -> String {
        let fullName = getFullName()
        if fullName.lengthOfBytes(using: .utf8) <= maxSize {
            return fullName
        }
        let firstName = firstName.components(separatedBy: " ")[0]
        return firstName.localizedCapitalized + " " + titleCaseName(lastName)
...
//    }
    ...
    public func getHashesForDatagroups( hashAlgorithm: String ) -> [DataGroupId:[UInt8]]  {
    ...
    public func verifyActiveAuthentication( challenge: [UInt8], signature: [UInt8] ) {
        ...
        // 
...
//                Log.debug("decryptedSig: \(binToHexRep(decryptedSig))")
//                if let pubkey = try? dg15.getPublicKeyBinary() {
//                    Log.debug("dg15 getPublicKeyBinary: \(binToHexRep(pubkey))")
//                }
                ...
                //    likely_sha1 = "8FE9258FFA4E00D45D3D1B5A23C2FA574882B573"
    ...
    private func ensureReadDataNotBeenTamperedWith( useCMSVerification: Bool ) throws  {
        ...
        var errors : String = ""
        for name in dataGroupsPresent {
            let id = DataGroupId.getIDFromName(name:name)
            ...
            //let computedHashVal = binToHexRep(dgVal.hash(sodHashAlgorithm))
            ...
            dataGroupHashes[id] = DataGroupHash(id: id.getName(), sodHash:sodHashVal)
        }
    ...
    /// Parses an text ASN1 structure, and extracts the Hash Algorithm and Hashes contained from the Octect strings
    /// - Parameter content: the text ASN1 stucure format
    /// - Returns: The Hash Algorithm used - either SHA1 or SHA256, and a dictionary of hashes for the datagroups (currently only DG1 and DG2 are handled)
    public func parseSODSignatureContent( _ content : String ) throws -> (String, [DataGroupId : String]){
        ...
        if sodHashAlgo == "" {
            throw PassiveAuthenticationError.UnableToParseSODHashes("Unable to find hash algorithm used" )
        }
