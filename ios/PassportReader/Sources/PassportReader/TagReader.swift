...
public class TagReader {
    ...
    func readDataGroup( dataGroup: DataGroupId ) async throws -> [UInt8]  {
        guard let tag = dataGroup.getFileIDTag() else {
            throw PassportReaderError.UnsupportedDataGroup
        }
...
//    func doInternalAuthentication( challenge: [UInt8] ) async throws -> ResponseAPDU {
    func doInternalAuthentication( challenge: [UInt8] ) async throws -> [UInt8] {
        let randNonce = Data(challenge)

        Log.debug("challenge len: \(challenge.count)")
        
        
        var data = [UInt8]()
        
        let dataCmd : [UInt8] = [0x00, 0x88, 0x00, 0x00, 0x08] + challenge +  [0xFF]
        let readCmd = NFCISO7816APDU(data:Data(dataCmd))!
//        let readCmd = NFCISO7816APDU(data:Data([0x00, 0x88, 0x00, 0x00, 0x00, 0x00,0x04]))!
        var resp = try await send( cmd: readCmd )
        Log.debug("got resp len: \(resp.data.count)")
        return resp.data
        
//        let (len, o) = try! asn1Length([UInt8](resp.data[1..<4]))
//        var remaining = Int(len)
//        Log.debug("possibly remaining: \(remaining)")
...
//        
//        
////        let cmd: NFCISO7816APDU = NFCISO7816APDU(instructionClass: 00, instructionCode: 0x88, p1Parameter: 0, p2Parameter: 0, data: randNonce, expectedResponseLength: 256)
//        var cmd: NFCISO7816APDU = NFCISO7816APDU(instructionClass: 00, instructionCode: 0x88, p1Parameter: 0, p2Parameter: 0, data: randNonce, expectedResponseLength: 224)
//        var resp = try await send( cmd: cmd )
//        Log.debug("got resp len: \(resp.data.count)")
//        
////        let (len, o) = try! asn1Length([UInt8](resp.data[1..<4]))
////        var remaining = Int(len)
...
//        data += resp.data
//        
////        let offset = intToBin(60, pad:4)
////        
////        cmd = NFCISO7816APDU(instructionClass: 00, instructionCode: 0x88,
////             p1Parameter: 00,
////             p2Parameter: 0,
////             data: Data(), expectedResponseLength: 128)
...
//        return data
    ...
    func sendGeneralAuthenticate( data : [UInt8], lengthExpected : Int = 256, isLast: Bool) async throws -> ResponseAPDU {
        ...
        do {
            ...
            if case PassportReaderError.ResponseError(_, let sw1, let sw2) = error,
               sw1 == 0x67, sw2 == 0x00 {
    ...
    func selectFileAndRead( tag: [UInt8]) async throws -> [UInt8] {
        ...
        guard let readHeaderCmd = NFCISO7816APDU(data:Data([0x00, 0xB0, 0x00, 0x00, 0x00, 0x00,0x04])) else {
            throw PassportReaderError.UnexpectedError
        }
        ...
        while remaining > 0 {
            ...
            do {
                ...
                readRetryCount = 0 // Reset retry count on success
            } catch {
                // Check if this is an NFC communication error (tag response error / no response)
                let errorDescription = error.localizedDescription
                if errorDescription.contains("Tag response error") || 
                   errorDescription.contains("no response") ||
                   errorDescription.contains("102") {
                    readRetryCount += 1
                    Log.warning("TagReader - NFC communication error, retry \(readRetryCount)/\(maxReadRetries): \(errorDescription)")
                    
                    if readRetryCount < maxReadRetries {
                        // Add a small delay before retrying to let the chip recover
            ...
            // Add a small delay between reads to prevent overwhelming the chip
            if remaining > 0 {
                try await Task.sleep(nanoseconds: 10_000_000) // 10ms delay between reads
            }
        }
...
//
//    func tryReadingActiveAuth(challenge: [UInt8]) async throws -> ResponseAPDU {
//        
//        let randNonce = Data(challenge)
//
//        let cmd: NFCISO7816APDU = NFCISO7816APDU(instructionClass: 00, instructionCode: 0x88, p1Parameter: 0, p2Parameter: 0, data: randNonce, expectedResponseLength: 256)
...
//    }
    ...
    func readCardAccess(mode: CardAccessReadMode = .ISO7816) async throws -> [UInt8] {
        ...
        do {
            ...
            // If ISO7816 mode failed, try ICAO9303 mode
            if mode == .ISO7816 {
                Log.debug("ISO7816 mode failed, trying ICAO9303 mode")
                return try await readCardAccess(mode: .ICAO9303)
            }
            throw error
        }

        // Try alternative CardAccess locations
        // CardAccess must contain valid ASN.1 PACE parameters, typically at least 15-20 bytes
        let minCardAccessSize = 10
        let possibleLocations: [[UInt8]] = [
            [0x01, 0x1C],  // Standard location (EF.CardAccess)
            [0x01, 0x1D],  // Alternative 1
            [0x2F, 0x01],  // Alternative 2 (EF.DIR) - less likely but worth trying
        ]

        for location in possibleLocations {
            do {
                let locationStr = location.map { String(format: "%02X", $0) }.joined()
                Log.debug("Trying CardAccess at \(locationStr)")
                let data = try await self.selectFileAndRead(tag: location)
                
                // Validate that the data is large enough to contain PACE parameters
                if data.count < minCardAccessSize {
                    Log.debug("Data at \(locationStr) is too small (\(data.count) bytes) - skipping")
                    continue
                }
                
                Log.debug("Found CardAccess at \(locationStr) with \(data.count) bytes")
                return data
            } catch {
                Log.debug("CardAccess not at \(location.map { String(format: "%02X", $0) }.joined()): \(error)")
                continue
            }
        }

        // If we've tried all locations in ISO7816 mode, try ICAO9303 mode
        if mode == .ISO7816 {
            Log.debug("All CardAccess locations failed with ISO7816 mode, trying ICAO9303 mode")
            return try await readCardAccess(mode: .ICAO9303)
        }

        throw PassportReaderError.ResponseError("CardAccess not found at any known location", 0x6A, 0x82)
    }
    ...
    func send( cmd: NFCISO7816APDU ) async throws -> ResponseAPDU {
        ...
        if rep.sw1 != 0x90 && rep.sw2 != 0x00 {
            ...
            let tagError: PassportReaderError
            if ((rep.sw1 == 0x63 && rep.sw2 == 0x00) || (rep.sw1 == 0x63 && rep.sw2 >= 0xC0 && rep.sw2 <= 0xCF)) {
                tagError = PassportReaderError.InvalidMRZKey
            } else {
                let errorMsg = self.decodeError(sw1: rep.sw1, sw2: rep.sw2)
                Log.error( "reason: \(errorMsg)" )
                tagError = PassportReaderError.ResponseError( errorMsg, sw1, sw2 )
            }
            throw tagError
        }
