...
public class OpenSSLUtils {
    ...
    // TODO: Try similar but for ECDSA for an ecdsaPublicKeyToUInt8Array implementation
    static func rsaPublicKeyToUInt8Array(pubKey: OpaquePointer) -> [UInt8]? {
        guard let rsa = EVP_PKEY_get1_RSA(pubKey) else { return nil }
        defer { RSA_free(rsa) }

        let len = i2d_RSAPublicKey(rsa, nil)
        if len <= 0 { return nil }

        var buffer: UnsafeMutablePointer<UInt8>?
        let _ = i2d_RSAPublicKey(rsa, &buffer)
        guard let buf = buffer else { return nil }

        let result = Array(UnsafeBufferPointer(start: buf, count: Int(len)))
        free(buf)
    ...
    @available(iOS 13, macOS 10.15, *)
    public static func getRSAPublicKeyData(from key:OpaquePointer) -> (modulus: [UInt8]?, exponent: [UInt8]?) {
        var modulusData : [UInt8] = []
        var exponentData : [UInt8] = []
        // Get Key type
        let v = EVP_PKEY_get_base_id( key )
        if v == EVP_PKEY_RSA || v == EVP_PKEY_RSA2 || v == EVP_PKEY_RSA_PSS {
    
            guard let rsa = EVP_PKEY_get0_RSA(key) else {
            ...
            let modulusCount = (BN_num_bits(n)+7)/8
            modulusData = [UInt8](repeating: 0, count: Int(modulusCount))
            _ = BN_bn2bin(n, &modulusData)

            let exponentCount = (BN_num_bits(e)+7)/8
            exponentData = [UInt8](repeating: 0, count: Int(exponentCount))
            _ = BN_bn2bin(e, &exponentData)
        }
        
        return (modulus: modulusData, exponent: exponentData)
    }

    @available(iOS 13, macOS 10.15, *)
    public static func getECDSAPublicKeyData(from key:OpaquePointer) -> (x: [UInt8]?, y: [UInt8]?) {
        var xData : [UInt8] = []
        var yData : [UInt8] = []
        ...
        if v == EVP_PKEY_EC {
            ...
            let form = EC_KEY_get_conv_form(ec)
            var len = EC_POINT_point2oct(ec_group, ec_pub, form, nil, 0, nil)
            xData = [UInt8](repeating: 0, count: Int(len))
            _ = EC_POINT_point2oct(ec_group, ec_pub, form, &xData, len, nil)

            len = EC_POINT_point2oct(ec_group, ec_pub, form, nil, 0, nil)
            yData = [UInt8](repeating: 0, count: Int(len))
            _ = EC_POINT_point2oct(ec_group, ec_pub, form, &yData, len, nil)
        }
    ...
    /*@available(iOS 13, macOS 10.15, *)
    public static func getECDSACurve(from key:OpaquePointer) -> String? {
        let v = EVP_PKEY_get_base_id( key )
        if v == EVP_PKEY_EC {
            return EC_KEY_get_curve_name(EVP_PKEY_get0_EC_KEY(key))
        }
        return nil
    }*/
