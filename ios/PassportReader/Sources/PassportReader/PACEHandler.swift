...
extension PACEHandlerError: LocalizedError {
    public var errorDescription: String? {
        switch self {
            case .DHKeyAgreementError(let errMsg): 
                return "DH key agreement error: \(errMsg)".localized(withComment: "DH key error")
            case .ECDHKeyAgreementError(let errMsg): 
                return "ECDH key agreement error: \(errMsg)".localized(withComment: "ECDH key error")
        }
    }
}
...
public class PACEHandler {
    ...
    public init(cardAccess : CardAccess, tagReader: TagReader) throws {
        ...
        guard let pi = cardAccess.paceInfo else {
            throw PassportReaderError.NotYetSupported( "PACE not supported" )
        }
...
*/
    ...
    func doStep1() async throws -> [UInt8] {
        ...
        if self.cipherAlg == "DESede" {
            ...
            throw PassportReaderError.UnsupportedCipherAlgorithm
        }
    ...
    func doStep2( passportNonce: [UInt8]) async throws -> OpaquePointer {
        ...
        switch(mappingType) {
            ...
            default:
                throw PassportReaderError.PACEError( "Step2GM", "Unsupported Mapping Type" )
        }
    ...
    func doPACEStep2GM(passportNonce : [UInt8]) async throws -> OpaquePointer {
        ...
        guard let pcdMappingEncodedPublicKey = OpenSSLUtils.getPublicKeyData(from: mappingKey) else {
            throw PassportReaderError.PACEError( "Step2GM", "Unable to get public key from mapping key")
        }
        ...
        guard let bn_nonce = BN_bin2bn(passportNonce, Int32(passportNonce.count), nil) else {
            throw PassportReaderError.PACEError( "Step2GM", "Unable to convert picc nonce to bignum" )
        }
        ...
        if self.agreementAlg == "DH" {
            ...
            throw PassportReaderError.PACEError( "Step2GM", "Unsupported agreement algorithm" )
        }
    ...
    func doPACEStep2IM( passportNonce: [UInt8] ) async throws -> OpaquePointer {
        // Not implemented yet
        throw PassportReaderError.PACEError( "Step2IM", "IM not yet implemented" )
    }
...
///         - Tuple of Generated Ephemeral KeyPair and the Passport's public key
    func doStep3KeyExchange(ephemeralParams: OpaquePointer) async throws -> (OpaquePointer, OpaquePointer) {
        ...
        guard let ephEcKey = EC_KEY_new() else {
            throw PassportReaderError.PACEError( "Step3 KeyEx", "Failed to create EC key" )
        }
        ...
        guard
            ...
            throw PassportReaderError.PACEError( "Step3 KeyEx", "Failed to generate EC key" )
        }

        // Wrap the EC_KEY into an EVP_PKEY
        var ephKeyPair = EVP_PKEY_new()
        guard let ephemeralKeyPair = ephKeyPair else {
            throw PassportReaderError.PACEError( "Step3 KeyEx", "Unable to get create ephermeral key pair" )
        }
        defer { EVP_PKEY_free(ephKeyPair) }

        guard EVP_PKEY_set1_EC_KEY(ephemeralKeyPair, ephEcKey) == 1 else {
            throw PassportReaderError.PACEError( "Step3 KeyEx", "Failed to set ephemeral key pair private key" )
        }
        
        Log.debug( "Generated Ephemeral key pair")

        guard let publicKey = OpenSSLUtils.getPublicKeyData( from: ephemeralKeyPair ) else {
            throw PassportReaderError.PACEError( "Step3 KeyEx", "Unable to get public key from ephermeral key pair" )
        }
        ...
        guard let passportPublicKey = OpenSSLUtils.decodePublicKeyFromBytes(pubKeyData: passportEncodedPublicKey!, params: ephemeralKeyPair) else {
            throw PassportReaderError.PACEError( "Step3 KeyEx", "Unable to decode passports ephemeral key" )
        }
    ...
    func doStep4KeyAgreement( pcdKeyPair: OpaquePointer, passportPublicKey: OpaquePointer) async throws -> ([UInt8], [UInt8]) {
        ...
        guard let pcdAuthToken = try? generateAuthenticationToken( publicKey: passportPublicKey, macKey: macKey) else {
            throw PassportReaderError.PACEError( "Step3 KeyAgreement", "Unable to generate authentication token using passports public key" )
        }
        ...
        guard piccToken == expectedPICCToken else {
            ...
            throw PassportReaderError.PACEError( "Step3 KeyAgreement", "Error PICC Token mismatch!\npicToken - \(piccToken)\nexpectedPICCToken - \(expectedPICCToken)" )
        }
    ...
    func paceCompleted( ksEnc: [UInt8], ksMac: [UInt8] ) throws {
        ...
        if (cipherAlg.hasPrefix("DESede")) {
            ...
            throw PassportReaderError.PACEError( "PACECompleted", "Not restarting secure messaging as unsupported cipher algorithm requested - \(cipherAlg)" )
        }
    }
}
...
extension PACEHandler {
    ...
    func encodePublicKey( oid : String, key : OpaquePointer ) throws -> [UInt8] {
        ...
        guard let pubKeyData = OpenSSLUtils.getPublicKeyData(from: key) else {
            ...
            throw PassportReaderError.InvalidDataPassed("Unable to get public key data")
        }
